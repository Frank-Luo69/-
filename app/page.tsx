...
export default function Page() {
  ...
  const advice = useMemo(
    () => (result ? adviseZhuXi(result.lines) : null),
    [result]
  );

  // 由种子字符串生成确定性 RNG；留空时用安全随机
  const rng = useMemo(() => {
  ...
  const handleCast = () => {
    ...
    try {
      saveHistory({
        ...
        seed: seedText || "(secure-random)",
        question,
        lines: r.lines as any,
        relating: r.relating as any,
      });
    } catch {}
  };
  ...
  return (
    <main className="mx-auto max-w-6xl p-6">
      ...
      <p className="text-gray-600 mb-6">
        蓍草法“三变成一爻、六爻成卦”严谨复现。留空使用安全随机；输入种子以复现结果。
      </p>
...
{JSON.stringify(
  ...
  2
          ...
          <ClassicTexts
            lines={result.lines}
            primary={advice?.primaryLines || []}
            allMoving={(advice?.movingCount || 0) === 6}
          />
          <GlossaryList />
          <HistoryPanel
            onRestore={(s, q) => {
